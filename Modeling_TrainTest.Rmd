...
# Apicar a técnica Smote para balanceamento 

```{r}
X <- x_train
y <- as.numeric(ifelse(y_train == 3, 1, 0))
smote_result <- SMOTE(X, y, K = 5, dup_size = 1)  # dup_size=1 → 100% oversampling da minoria
...
auc_value <- round(as.numeric(roc_obj$auc) * 100, 2)  # Área sob a curva (AUC)
...
huberty_index(as.numeric(confusion_mat$overall[1]), max(prop.table(table(y_test)))) # Índice de Huberty 
```
...
# 📌 Converte os dados em formato de matriz para o XGBoost
...
# 📌 Define os parâmetros do modelo XGBoost para classificação binária
params <- list(
  objective = "binary:logistic",  # Para classificação binária
  eval_metric = "logloss",  # Métrica de erro logarítmico
  eta = 0.3,  # Taxa de aprendizado
  max_depth = 3,  # Profundidade da árvore
  subsample = 0.8,  # Amostragem aleatória de observações
  colsample_bytree = 0.8,  # Amostragem aleatória de variáveis
  scale_pos_weight = sum(train$Sets == 0) / sum(train$Sets == 1)  # Ajusta o peso da classe minoritária
)
...
# 📌 Treina o modelo SVM com Kernel RBF
svm_model <- svm(Sets ~ .,  # Variável alvo vs. todas as preditoras
                 data = train, 
                 probability = T,
